...
resource "kubernetes_secret" "prod_iiif_tls_secret" {
  metadata {
    name      = "prod-iiif-tls"
    namespace = kubernetes_namespace.namespace.metadata[0].name
  }
  ...
  data = {
    "tls.crt" = "${base64decode(var.prod_iiif_tls_crt)}"
    "tls.key" = "${base64decode(var.prod_iiif_tls_key)}"
  }
...
resource "kubernetes_ingress" "prod-iiif" {
  metadata {
    name = "iiif"
    namespace = kubernetes_namespace.namespace.metadata[0].name
    annotations = {
  ...
  spec {
    tls {
      secret_name = kubernetes_secret.prod_iiif_tls_secret.metadata[0].name
      hosts = ["iiif.library.ucla.edu"]
    }
