...
resource "kubernetes_secret" "test_iiif_tls_secret" {
  metadata {
    name      = "test-iiif-tls"
    namespace = kubernetes_namespace.namespace.metadata[0].name
  }
  ...
  data = {
    "tls.crt" = "${base64decode(var.test_iiif_tls_crt)}"
    "tls.key" = "${base64decode(var.test_iiif_tls_key)}"
  }
...
resource "kubernetes_ingress" "test-iiif" {
  ...
  spec {
    tls {
      secret_name = kubernetes_secret.test_iiif_tls_secret.metadata[0].name
